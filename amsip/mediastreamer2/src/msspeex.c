...
static int enc_get_statistics(MSFilter *f, void *arg){
	int i;
...
static MSFilterMethod enc_methods[]={
	...
	{	MS_FILTER_GET_STATISTICS, enc_get_statistics },
	{	0				,	NULL		}
};
...
static void dec_init(MSFilter *f){
	SpeexDecState *s=(SpeexDecState *)ms_new(SpeexDecState,1);
	s->rate=8000;
	s->frsz=0;
	s->state=NULL;
	s->penh=1;
...
static void dec_uninit(MSFilter *f){
	SpeexDecState *s=(SpeexDecState*)f->data;
    if (s==NULL)
		return;
    if (s->state!=NULL)
...
static int dec_get_statistics(MSFilter *f, void *arg){
	int i;
	ms_warning("filter: %s[%i->%i]", f->desc->name,
		f->desc->ninputs, f->desc->noutputs);
	for (i=0;i<f->desc->ninputs;i++) {
...
static MSFilterMethod dec_methods[]={
	...
	{	MS_FILTER_GET_STATISTICS, dec_get_statistics },
	{	0				,	NULL		}
};
