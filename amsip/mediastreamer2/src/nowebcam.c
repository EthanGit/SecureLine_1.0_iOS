...
#ifndef MS2_MINIMAL_SIZE
	m=ms_yuv_load_mjpeg((uint8_t*)&def_mire,sizeof(def_mire),reqsize);
	if (m!=NULL)
	{
		return m;
	}
...
mblk_t *ms_load_jpeg_as_yuv(const char *jpgpath, MSVideoSize *reqsize){
	...
	m=ms_yuv_load_mjpeg(jpgbuf,st_sizel,reqsize);
	ms_free(jpgbuf);
	if (m==NULL)
	{
		CloseHandle(fd);
	...
	if (fd!=-1){
		...
		err=(int)read(fd,jpgbuf,statbuf.st_size);
		if (err!=statbuf.st_size){
			ms_error("Could not read as much as wanted: %i<>%i !",err,statbuf.st_size);
		}
		m=ms_yuv_load_mjpeg(jpgbuf,(int)statbuf.st_size,reqsize);
		ms_free(jpgbuf);
		if (m==NULL)
		{
			close(fd);
...
static void static_image_init(MSFilter *f){
...
static void static_image_uninit(MSFilter *f){
	SIData *d=(SIData*)f->data;
	ms_free(d->nowebcamimage);
	ms_free(d);
}

static void static_image_preprocess(MSFilter *f){
	SIData *d=(SIData*)f->data;
	if (d->pic==NULL){
...
static void static_image_process(MSFilter *f){
	SIData *d=(SIData*)f->data;
...
static void static_image_postprocess(MSFilter *f){
	SIData *d=(SIData*)f->data;
	if (d->pic) {
		freemsg(d->pic);
		d->pic=NULL;
...
static int static_image_set_vsize(MSFilter *f, void* data){
	SIData *d=(SIData*)f->data;
	d->vsize=*(MSVideoSize*)data;
	return 0;
}

static int static_image_get_vsize(MSFilter *f, void* data){
	SIData *d=(SIData*)f->data;
	*(MSVideoSize*)data=d->vsize;
	return 0;
}

static int static_image_get_pix_fmt(MSFilter *f, void *data){
