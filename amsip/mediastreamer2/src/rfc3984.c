...
#define TYPE_FU_B 29    /*fragmented unit 0x1D*/
#define TYPE_STAP_A 24  /*single time aggregation packet  0x18*/
#define TYPE_STAP_B 25  /*single time aggregation packet  0x19*/
#define TYPE_MTAP_16 26  /*single time aggregation packet  0x1A*/
#define TYPE_MTAP_24 27  /*single time aggregation packet  0x1B*/
...
void rfc3984_unpack(Rfc3984Context *ctx, mblk_t *im, MSQueue *out){
	...
	}else if (type==TYPE_STAP_B){
		ms_warning("Receiving STAP-B");
		freemsg(im);
	}else if (type==TYPE_FU_B){
		ms_warning("Receiving FU-B");
		freemsg(im);
	}else if (type==TYPE_MTAP_16){
		ms_warning("Receiving MTAP_16");
		freemsg(im);
	}else if (type==TYPE_MTAP_24){
		ms_warning("Receiving MTAP_24");
		freemsg(im);
	}else if (type==30 || type==31 || type==0){
		ms_warning("Receiving undefined nal unit type");
		freemsg(im);
	}else{
		if (ctx->m){
