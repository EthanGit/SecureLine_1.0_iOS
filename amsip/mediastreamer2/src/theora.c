...
static void enc_init(MSFilter *f){
	TheoraEncState *s=(TheoraEncState *)ms_new(TheoraEncState,1);
...
static void enc_uninit(MSFilter *f){
	TheoraEncState *s=(TheoraEncState*)f->data;
...
static int create_packed_conf(TheoraEncState *s){
	ogg_packet p;
...
static void packetize_and_send(MSFilter *f, TheoraEncState *s, mblk_t *om, uint32_t timestamp, uint8_t tdt){
	mblk_t *packet;
	mblk_t *h;
	int npackets=0;
...
bool_t need_send_conf(TheoraEncState *s, uint64_t elapsed){
...
static void dec_init(MSFilter *f){
	TheoraDecState *s=(TheoraDecState *)ms_new(TheoraDecState,1);
	s->ready=FALSE;
	theora_info_init(&s->tinfo);
	s->yuv=NULL;
	s->curframe=NULL;
...
static void dec_uninit(MSFilter *f){
	TheoraDecState *s=(TheoraDecState*)f->data;
...
static bool_t dec_init_theora(TheoraDecState *s, ogg_packet *op){
	theora_comment tcom;
...
static mblk_t *dec_unpacketize(MSFilter *f, TheoraDecState *s, mblk_t *im, int *tdt){
	uint8_t ft;
...
static void dec_process_frame(MSFilter *f, TheoraDecState *s, ogg_packet *op){
	yuv_buffer yuv;
