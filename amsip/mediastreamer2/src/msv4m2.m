...
(QTCaptureConnection *)connection
{
	...
	int size = (int)[sampleBuffer lengthForAllSamples];
	buf=allocb(size,0);
	...
	{	
		...
		while ((desc = [enumerator nextObject])) 
		{
			if ([desc mediaType] == QTMediaTypeVideo)
			{
				...
				if (fmt > 0x28)
					NSLog(@"format for webcam: %c%c%c%c", (char)((fmt >> 24) & 0xFF),
											  (char)((fmt >> 16) & 0xFF),
											  (char)((fmt >>  8) & 0xFF),
											  (char)((fmt >>  0) & 0xFF));
				else
					NSLog(@"format for webcam: %d", fmt);
				if( fmt == k2vuyPixelFormat) //k422YpCbCr8CodecType)
				{	
					NSLog(@"qtcapturedevice using MS_UYVY");
					return MS_UYVY;
				}
				//else if( fmt == kOpenDMLJPEGCodecType)
				//{
				//  Not working with my cam... seems I get no data.
				//	NSLog(@"qtcapturedevice using MS_MJPEG - kOpenDMLJPEGCodecType");
				//	return MS_MJPEG;
				//}
				else if( fmt == kCVPixelFormatType_420YpCbCr8Planar)
				{	
					NSLog(@"qtcapturedevice using MS_YUV420P");
					return MS_YUV420P;
				}
				
				else if( fmt == kCVPixelFormatType_24RGB)
				{	
					NSLog(@"qtcapturedevice using MS_RGB24");
					return MS_RGB24;
				}
				...
				{	
					NSLog(@"qtcapturedevice using MS_YUY2");
					return MS_YUY2;
				}				
				else if( fmt == k32RGBAPixelFormat)
				{	
					NSLog(@"qtcapturedevice using MS_RGBA32");
					return MS_RGBA32;
				}				
            }
        }
		NSLog(@"qtcapturedevice no format detected");
		NSDictionary * dic = [NSDictionary dictionaryWithObjectsAndKeys:
							  [NSNumber numberWithInteger:k32RGBAPixelFormat], (id)kCVPixelBufferPixelFormatTypeKey,
							  nil];
		[output setPixelBufferAttributes:dic];
		return MS_YUV420P;
    }
	
	NSLog(@"qtcapturedevice fallback to MS_YUV420P");
	return MS_YUV420P;
}
...
-(void) setName:(char*) name
{
	...
	if(name != nil)
	{
		...
		for(i = 0 ; i < [array count]; i++)
		{
			...
			NSLog(@"device name = %s", [[deviceTmp localizedDisplayName] UTF8String]);
			if(strcmp([[deviceTmp localizedDisplayName] UTF8String], name)==0)
			{
				device = deviceTmp;
				break;
			}
...
-(MSVideoSize) getSize
{
	...
	if(output)
	{
		...
		size.width = (int)[[dic objectForKey:(id)kCVPixelBufferWidthKey] integerValue];
		size.height = (int)[[dic objectForKey:(id)kCVPixelBufferHeightKey] integerValue];
	}
...
static void ms_v4m_detect(MSWebCamManager *obj){
	...
	for(i = 0 ; i < [array count]; i++)
	{
		...
		NSArray *format_array = [device formatDescriptions];
