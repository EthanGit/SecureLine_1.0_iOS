...
static void cam_update(MSWebCamManager *m, MSWebCamDesc *desc){
	if (desc->update!=NULL)
	{
		MSList *elem;
		for (elem=m->cams;elem!=NULL;elem=elem->next){
			MSWebCam *cam=(MSWebCam*)elem->data;
			if (strcmp(ms_web_cam_get_driver_type(cam),desc->driver_type)==0)
				cam->disconnected=1;
		}
...
void ms_web_cam_manager_update(MSWebCamManager *m){
	MSList *elem;
	for(elem=m->descs;elem!=NULL;elem=elem->next)
		cam_update(m,(MSWebCamDesc*)elem->data);
}
