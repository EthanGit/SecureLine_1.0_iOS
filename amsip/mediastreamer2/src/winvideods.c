...
static void vfw_update(MSWebCamManager *obj);
...
static void vfw_update(MSWebCamManager *obj){
	...
	while(S_OK == pEnumMoniker->Next(1, &pMoniker, &nFetched) )
	{
		...
		char *idname = ms_strdup_printf("%s: %s", ms_directx_cam_desc.driver_type,szName);
		MSWebCam *cam=ms_web_cam_manager_get_cam(obj, idname);
		ms_free(idname);
		if (cam==NULL)
		{
			IBaseFilter *m_pDeviceFilter;
			...
			/* confirm that webcam still exist */
			cam->disconnected=0;
		}
