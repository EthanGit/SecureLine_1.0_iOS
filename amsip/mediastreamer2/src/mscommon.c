...
static MSList *ms_list_new(void *data){
...
int ms_load_plugins(const char *dir){
	...
	while (!fFinished) 
	{
		...
		os_handle = LoadLibraryEx (wUnicodePluginFile, NULL, LOAD_WITH_ALTERED_SEARCH_PATH);
		if (os_handle==NULL)
		{
			ms_message("Fail to load plugin %s with altered search path: error %i",szPluginFile,GetLastError());
			os_handle = LoadLibraryEx (wUnicodePluginFile, NULL, 0);
		}
		...
		if (os_handle==NULL)
			...
			WideCharToMultiByte(CP_UTF8,0,FileData.cFileName,-1,szPluginName,256,0,0);
	...
	ds=opendir(dir);	
	if (ds==NULL){
		ms_message("Cannot open directory %s: %s",dir,ortp_strerror(errno));
		return -1;
	}
...
#define DEFAULT_MAX_PAYLOAD_SIZE 1408
