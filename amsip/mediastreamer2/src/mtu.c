...
int ms_discover_mtu(const char *host)
{
  ...
  if (!m_IcmpInst)
	{
		m_IcmpInst = LoadLibrary(L"icmp.dll");
		if (m_IcmpInst)
		{
...
int ms_discover_mtu(const char *host){
	...
	if(sock < 0)
	{
		ms_error("socket(): %s",ortp_strerror(errno));
		return sock;
	}
	mtu=IP_PMTUDISC_DO;
	optlen=sizeof(mtu);
	...
	if (err!=0){
		ms_error("setsockopt(): %s",ortp_strerror(errno));
		err = close(sock);
		if (err!=0)
			ms_error("close(): %s", ortp_strerror(errno));
		return -1;
	}
	err=connect(sock,ai->ai_addr,ai->ai_addrlen);
	freeaddrinfo(ai);
	if (err!=0){
		ms_error("connect(): %s",ortp_strerror(errno));
		err = close(sock);
		if (err !=0)
			ms_error("close(): %s", ortp_strerror(errno));
		return -1;
	...
	do{
		...
		if(buf == NULL)
		{
			ms_error("malloc(): %s",ortp_strerror(errno));
			err = close(sock);
			if (err!=0)
				ms_error("close(): %s", ortp_strerror(errno));
			return -1;
		...
		if (err!=0){
			ms_error("getsockopt(): %s",ortp_strerror(errno));
			err = close(sock);
			if (err!=0)
				ms_error("close(): %s", ortp_strerror(errno));
			return -1;
...
void ms_set_mtu(int mtu){
	...
	if (mtu>60){
		if (mtu>1468) mtu=1468;/*limit to 1468, the mediastreamer2 buffer are not large enough anyway*/
