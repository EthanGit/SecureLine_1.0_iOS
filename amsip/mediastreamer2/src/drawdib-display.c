...
static void yuv2rgb_uninit(Yuv2RgbCtx *ctx){
	...
	if (ctx->sws){
		ms_video_scalercontext_free(ctx->sws);
		ctx->sws=NULL;
	}
	ctx->dsize.width=0;
	ctx->dsize.height=0;
...
static void yuv2rgb_prepare(Yuv2RgbCtx *ctx, MSVideoSize src, MSVideoSize dst){
	...
	ctx->sws=ms_video_scalercontext_init(src.width,src.height,MS_YUV420P,
...
static void yuv2rgb_process(Yuv2RgbCtx *ctx, MSPicture *src, MSVideoSize dstsize, bool_t mirroring){
	...
	}
	{
		...
		if (ms_video_scalercontext_convert(ctx->sws,src->planes,src->strides, 0,
...
static void dd_display_prepare(MSFilter *f){
	...
	if (dd->window==NULL){
		...
		SetWindowLongPtr(dd->window, GWLP_USERDATA, (LONG_PTR)dd);
	}
	if (dd->ddh==NULL)
		dd->ddh=DrawDibOpen();
}
