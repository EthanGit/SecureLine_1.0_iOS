...
typedef struct Volume{
	float energy;
	float norm_en;
	float previous_gain; /*the one really applied, smoothed by noise gate and echo limiter*/
...
static void volume_init(MSFilter *f){
	...
	v->energy=0;
	v->norm_en=0;
	v->static_gain=v->gain=v->previous_gain=1;
	v->ea_active=FALSE;
	v->gain_k=gain_k;
	v->thres=noise_thres;
	v->force=en_weight;
...
static void volume_echo_avoider_process(Volume *v, uint64_t curtime){
	...
	if (v->ea_active){
		if (peer_e>v->thres){
			...
			{
				//only allow small variation every hop
				//to avoid large artifacts
				if (v->gain>gain && v->gain-gain>0.25f)
					v->gain=v->gain-0.25f;
				else if (gain>v->gain && gain-v->gain>0.25f)
					v->gain=v->gain+0.25f;
				else
					v->gain=gain;
			}
...
static void apply_gain(Volume *v, mblk_t *m){
	...
	if (gain==1 && v->gain==1 && v->previous_gain==1) return;
	v->gain=(v->gain*(1-v->gain_k)) + (v->gain_k*gain);

	if (v->previous_gain-v->gain>0.1f
		|| v->previous_gain-v->gain<-0.1f)
	{
		float len=m->b_wptr-m->b_rptr;
		float pos=0;
		for (	sample=(int16_t*)m->b_rptr;
			...
			if (pos<len/2)
			{
				t_gain = sqrt(sqrt((2*(len/2-pos)/len)));
				gain = v->previous_gain + ((v->gain-v->previous_gain)/2)*(1-t_gain);
			}
			else
			{
				t_gain = sqrt(sqrt((2*(pos-len/2)/len)));
				gain = v->gain +((v->previous_gain-v->gain)/2)*(1-t_gain);
			}
			pos++;
			pos++;
...
static void volume_process(MSFilter *f){
	...
	if (v->agc_enabled){
		...
		while((m=ms_queue_get(f->inputs[0]))!=NULL){
			en=update_energy((int16_t*)m->b_rptr,(int)((m->b_wptr-m->b_rptr)/2),en);
			v->previous_gain = v->gain;
			if (v->peer){
				volume_echo_avoider_process(v,f->ticker->time);
			}else v->target_gain=v->static_gain;
