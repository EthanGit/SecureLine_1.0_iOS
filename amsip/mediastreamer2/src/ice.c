...
struct IceData {
	...
	int last_packet_sent_tick;
	int stun_indication_received;
};

typedef struct IceData IceData_t;
...
static int ice_sound_send_stun_request(RtpSession *session, struct IceCheckList *checklist, uint64_t ctime)
{
...

	{
		...
		ortp_socket_t media_socket = rtp_session_get_rtp_socket(session);
...
_ice_createErrorResponse(StunMessage *response, int cl, int number, const char* msg)
{
	...
	response->errorCode.sizeReason = (uint16_t)strlen(response->errorCode.reason);
	response->hasFingerprint = TRUE;
}

static int ice_process_stun_message(IceData_t *d, RtpSession *session, struct IceCheckList *checklist, OrtpEvent *evt)
{
	struct CandidatePair *remote_candidates = NULL;
	StunMessage msg;
	bool_t res;
	...
	if (STUN_IS_INDICATION(msg.msgHdr.msgType))
	{
		...
		/* remote side has no STUN support, but remote side was able
		to connect to us using the candidate we have provided.
		NOTE: ssrc is not validated here... */
		if (checklist->rem_ice_ufrag[0]=='\0' ||
			remote_candidates[0].remote_candidate.conn_addr[0]=='\0' )
		{
			if (d->stun_indication_received==0) {
				if (evt_data->ep->addrlen!=session->rtp.rem_addrlen
					|| memcmp(&session->rtp.rem_addr,&evt_data->ep->addr, evt_data->ep->addrlen)!=0)
				{
					ms_message("ice.c: STUN INDICATION <- received but remote app has ice disabled (symmetric RTP used) (%s:%i)", src6host, recvport);
		...
		*/
		{
			...
			if (msg.hasFingerprint==TRUE)
			{
				...
				stunCalculateIntegrity_shortterm(hmac, (char*)mp->b_rptr, (int)(mp->b_wptr-mp->b_rptr-24-8), checklist->loc_ice_pwd);
			}
			else
				stunCalculateIntegrity_shortterm(hmac, (char*)mp->b_rptr, (int)(mp->b_wptr-mp->b_rptr-24), checklist->loc_ice_pwd);
...

		{
			...
			/* fake support for ICE: just use symmetric signalling */
			if (checklist->rem_ice_ufrag[0]=='\0' ||
				remote_candidates[0].remote_candidate.conn_addr[0]=='\0' )
			{
				if (evt_data->ep->addrlen!=session->rtp.rem_addrlen
					|| memcmp(&session->rtp.rem_addr,&evt_data->ep->addr, evt_data->ep->addrlen)!=0)
				{
					ms_message("ice.c: STUN REQ <- connectivity check received but remote app has ice disabled (symmetric RTP used) (%s:%i)", src6host, recvport);
					memcpy(&session->rtp.rem_addr, &evt_data->ep->addr, evt_data->ep->addrlen);
					session->rtp.rem_addrlen=evt_data->ep->addrlen;
					rtp_session_set_dscp(session, -1);
				}
	...
	else if (STUN_IS_SUCCESS_RESP(msg.msgHdr.msgType))
	{
		...
		res = stunParseMessage((char*)mp->b_rptr, (int)(mp->b_wptr-mp->b_rptr),
			&resp );
		if (!res)
		{
...
{
	IceData_t *d = (IceData_t *)ms_new(IceData_t, 1);

	d->ortp_event = ortp_ev_queue_new();
	d->session = NULL;
	d->check_lists = NULL;
	d->rate = 8000;
	d->last_packet_sent = 0;
	d->last_packet_sent_tick = -1;
	d->stun_indication_received = 0;
	f->data = d;
}

static void ice_postprocess(MSFilter * f)
{
	IceData_t *d = (IceData_t *) f->data;
...
static void ice_process(MSFilter * f)
{
		...
		while (evt != NULL) {
			if (ortp_event_get_type(evt) ==
				ORTP_EVENT_STUN_PACKET_RECEIVED) {
					ice_process_stun_message(d, d->session, d->check_lists, evt);
			}
			if (ortp_event_get_type(evt) ==
	...
	if (d->last_packet_sent_tick < 0)
	{
		d->last_packet_sent = d->session->rtp.stats.packet_sent;
		d->last_packet_sent_tick = f->ticker->ticks;
	}

	if (f->ticker->ticks-d->last_packet_sent_tick==50*2) {

		/* if no rtp sent, send a STUN INDICATION OVER THE SOCKET */
		/* This is a keep alive mechanism described in rfc6263 with no */
		/* impact on remote agent */
		if (d->last_packet_sent==d->session->rtp.stats.packet_sent) {
			StunMessage req;
			int error;
			char buf[STUN_MAX_MESSAGE_SIZE];
			int len = STUN_MAX_MESSAGE_SIZE;
			struct sockaddr *destaddr=(struct sockaddr*)&d->session->rtp.rem_addr;
			memset(&req, 0, sizeof(StunMessage));
			stunBuildReqSimple( &req, NULL, FALSE, FALSE, 1);
			req.msgHdr.msgType = (STUN_METHOD_BINDING|STUN_INDICATION);
			req.hasFingerprint = TRUE;
			len = stunEncodeMessage( &req, buf, len, NULL);
			error = sendto (d->session->rtp.socket, buf, len,
				 0,destaddr, d->session->rtp.rem_addrlen);
			if (error < 0)
				ortp_warning ("Error sending stun indication packet: socket=%i", d->session->rtp.socket);
		}
