...
#define NOWINDOW_DISPLAY
#undef HAVE_SDL

#if (TARGET_OS_IPHONE==0) && (TARGET_IPHONE_SIMULATOR==0)
//ADD THIS DEFINE TO DISPLAY PIN 0 and PIN 1 in ONE VIEW
#define NOWINDOW_DISPLAY_ONEVIEW
#endif
#if (TARGET_OS_IPHONE==0) && (TARGET_IPHONE_SIMULATOR==0)
//ADD THIS DEFINE TO RETREIVE PIN 0 and PIN 1 in YUY2 format
#define NOWINDOW_DISPLAY_UYVY
#endif

#else
/* #define NOWINDOW_DISPLAY */
#endif
/* #define NOWINDOW_DISPLAY_WITH_OUTPUT */
...
static bool_t sdl_display_init(MSDisplay *obj, MSFilter *f, MSPicture *fbuf, MSPicture *fbuf_selfview){
	...
	if (obj->window_id!=0)
	{
		//WindowRef window_ref = (WindowRef)obj->window_id;
		//ms_message("using windowref=%i", window_ref);
		//NSWindow *cocoa_window = [ [ NSWindow alloc ] initWithWindowRef:window_ref];
		NSView *cocoa_view = (NSView *)obj->window_id;
		NSWindow *cocoa_window = [cocoa_view window];
		if (cocoa_window==nil)
		{
			//ms_message("failure to get NSWindow (windowref=%i)", window_ref);
			wd->window = SDL_CreateWindow("Video",
			...
			//NSWindow *cocoa_window = (NSWindow *)obj->window_id;
			wd->window = SDL_CreateWindowFrom(cocoa_window);
		}
	...
	wd->renderer = SDL_CreateRenderer(wd->window, -1, 0);
	
	wd->texture = SDL_CreateTexture(wd->renderer,
	...
	if (i==0){
		...
		fbuf->h=wd->lay->h;
#else
		wd->fb_block = yuv_buf_alloc(&wd->fb, fbuf->w, fbuf->h);
...
static void sdl_display_lock(MSDisplay *obj){
	...
	
	{
		...
		uint32_t format;
		int access, w, h;
		SDL_QueryTexture(wd->texture, &format, &access, &w, &h);
		SDL_PixelFormatEnumToMasks(format, &bpp, &rmask, &gmask, &bmask, &amask);
		bpp = bpp / 8; /* convert to bytes per pixels */

		SDL_LockTexture(wd->texture, NULL, (void **) &surfptr, &pitch);
		
		ms_message("size of surface: %i", msgdsize(wd->fb_block));
		memcpy(surfptr, wd->fb_block->b_rptr, msgdsize(wd->fb_block));
	}		
...
static void sdl_display_update(MSDisplay *obj, int new_image, int new_selfview){
	...
	SDL_RenderCopy(wd->renderer,wd->texture, NULL, &rect);
...
#elif defined(NOWINDOW_DISPLAY)

typedef struct NoWindowDisplay{
	MSFilter *filter;
	MSPicture fb;
	MSPicture fb_selfview;
	uint8_t *rgb_selfview;
	int rgb_len_selfview;
	struct MSScalerContext *sws_selfview;
	uint8_t *rgb;
	int rgb_len;
...
static bool_t nowindow_display_init(MSDisplay *obj, MSFilter *f, MSPicture *fbuf, MSPicture *fbuf_selfview){
	NoWindowDisplay *wd=(NoWindowDisplay*)obj->data;
	int ysize,usize;
	...
	if (wd!=NULL)
	{
		...
		wd->rgb=NULL;
		wd->rgb_len=0;
		if (wd->sws!=NULL) ms_video_scalercontext_free(wd->sws);
		else
		{
			ms_message("We never had any mainview images");
		}
		wd->sws=NULL;
		...
		if (wd->sws_selfview!=NULL) ms_video_scalercontext_free(wd->sws_selfview);
		else
		{
			ms_message("We never had any selfview images");
		}
		wd->sws_selfview=NULL;
	}
	else
		wd=(NoWindowDisplay*)ms_new0(NoWindowDisplay,1);

#if defined(NOWINDOW_DISPLAY_UYVY)
	wd->format = MS_UYVY;
#elif defined(WIN32)
	wd->format = MS_RGB24;
#else
	...
	if (wd->format == MS_UYVY) {
		wd->rgb_len_selfview=ysize*2;
		wd->rgb_selfview=(uint8_t*)ms_malloc0(wd->rgb_len_selfview);
    } else if (wd->format == MS_RGBA) {
		wd->rgb_len_selfview=ysize*4;
		wd->rgb_selfview=(uint8_t*)ms_malloc0(wd->rgb_len_selfview);
	} else { /* MS_RGB24 */
...
static void yuv420p_to_rgb(NoWindowDisplay *wd, MSPicture *src, uint8_t *rgb){
	if (wd->format == MS_UYVY)
    {
		int rgb_stride;
		uint8_t *p;
		...
		if (wd->sws==NULL){
			wd->sws=ms_video_scalercontext_init(src->w,src->h,MS_YUV420P,
                                      src->w,src->h, MS_UYVY,
                                      MS_YUVFAST, NULL, NULL, NULL);
		}
		if (ms_video_scalercontext_convert(wd->sws,src->planes,src->strides, 0,
                         src->h, &p, &rgb_stride)<0){
            ms_error("Error in 420->rgb ms_video_scalercontext_convert().");
		}
    } else if (wd->format == MS_RGBA)
	{
		int rgb_stride;
...
static void yuv420p_to_rgb_selfview(NoWindowDisplay *wd, MSPicture *src, uint8_t *rgb){
	if (wd->format == MS_UYVY)
    {
		int rgb_stride;
		uint8_t *p;
		...
		if (wd->sws_selfview==NULL){
			wd->sws_selfview=ms_video_scalercontext_init(src->w,src->h,MS_YUV420P,
                                               src->w,src->h, MS_UYVY,
                                               MS_YUVFAST, NULL, NULL, NULL);
		}
		if (ms_video_scalercontext_convert(wd->sws_selfview,src->planes,src->strides, 0,
                         src->h, &p, &rgb_stride)<0){
            ms_error("Error in 420->rgb ms_video_scalercontext_convert().");
		}
    } else if (wd->format == MS_RGBA)
...
static void nowindow_display_update(MSDisplay *obj, int new_image, int new_selfview){
	...
	if (wd->format == MS_UYVY)
    {
        ...
        if (new_image>0)
        {        
            struct amsip_picture pic;
			yuv420p_to_rgb(wd, &wd->fb, wd->rgb);
            pic.width = wd->fb.w;
            pic.height = wd->fb.h;
            pic.format = wd->format;
            pic.size = wd->rgb_len;
            pic.pixels = wd->rgb;
            ms_filter_notify(wd->filter, MS_DISPLAY_NEW_IMAGE, (void*)&pic);
        }
        if (new_selfview>0)
        {
            struct amsip_picture pic;
            ...
            pic.width = wd->fb_selfview.w;
            pic.height = wd->fb_selfview.h;
            pic.format = wd->format;
            pic.size = wd->rgb_len_selfview;
            pic.pixels = wd->rgb_selfview;
            ms_filter_notify(wd->filter, MS_DISPLAY_NEW_IMAGE_SELFVIEW, (void*)&pic);
        }
        return;
    }
	...
	if (new_image>0)
	{
		if (wd->filter!=NULL)
		{
#if (!defined(NOWINDOW_DISPLAY_ONEVIEW)) ||  (TARGET_OS_IPHONE==1) || (TARGET_IPHONE_SIMULATOR==1)
			struct amsip_picture pic;
#endif
			yuv420p_to_rgb(wd, &wd->fb, wd->rgb);
...
#if defined(NOWINDOW_DISPLAY_ONEVIEW) && (TARGET_OS_IPHONE==0) && (TARGET_IPHONE_SIMULATOR==0)
	if (new_selfview>0 && new_image<=0)
		return;
	...
	if (new_image>0)
	{
		...
		if (corner>=0)
		{
			float sv_scalefactor=SCALE_FACTOR;
			int w_selfview;
			int h_selfview;
			int ratiow;
			int ratioh;
			...
			w_selfview = (int)(wd->fb_selfview.w/sv_scalefactor);
			w_selfview = w_selfview/ratiow*ratiow;
			h_selfview = w_selfview*ratioh/ratiow;
			
			if (wd->sws_selfview==NULL){
				wd->sws_selfview=ms_video_scalercontext_init(wd->fb_selfview.w,wd->fb_selfview.h,MS_YUV420P,
												   w_selfview,h_selfview, MS_ARGB,
												   MS_YUVFAST, NULL, NULL, NULL);
			}
			...
			{
				/*bottom right */
				rgb_stride=wd->fb.w*4;
				p=wd->rgb+(rgb_stride)*wd->fb.h-(rgb_stride)*h_selfview+(rgb_stride)-w_selfview*4;
			} else if (corner==2 || corner==6)
			{
				/*top right */
				rgb_stride=wd->fb.w*4;
				p=wd->rgb+(rgb_stride)-w_selfview*4;
			} else if (corner==3 || corner==7)
			{
				/* bottom left: OK */
				rgb_stride=wd->fb.w*4;
				p=wd->rgb+(rgb_stride)*wd->fb.h-(rgb_stride)*h_selfview;
			} else {
				/* top left: OK */
				rgb_stride=wd->fb_selfview.w*4;
				p=wd->rgb;
			}
			
			if (ms_video_scalercontext_convert(wd->sws_selfview,wd->fb_selfview.planes,wd->fb_selfview.strides, 0,
							 wd->fb_selfview.h, &p, &rgb_stride)<0){
				ms_error("Error in 420->rgb ms_video_scalercontext_convert().");
			}
		}
...
static void nowindow_display_uninit(MSDisplay *obj){
	NoWindowDisplay *wd=(NoWindowDisplay*)obj->data;
	if (wd==NULL)
		return;
	if (wd->fb_selfview.planes[0]) ms_free(wd->fb_selfview.planes[0]);
	if (wd->rgb_selfview) ms_free(wd->rgb_selfview);
	if (wd->sws_selfview) ms_video_scalercontext_free(wd->sws_selfview);
	if (wd->fb.planes[0]) ms_free(wd->fb.planes[0]);
	if (wd->rgb) ms_free(wd->rgb);
	if (wd->sws) ms_video_scalercontext_free(wd->sws);
	ms_free(wd);
}

static int nowindow_display_pollevent(MSDisplay *d, MSDisplayEvent *ev){
	return -1;
}
...
extern MSDisplayDesc ms_nowindow_display_desc={
	nowindow_display_init,
	NULL,
	NULL,
	nowindow_display_update,
	nowindow_display_uninit,
	nowindow_display_pollevent
};

#else

MSDisplayDesc ms_nowindow_display_desc={
	.init=nowindow_display_init,
	.update=nowindow_display_update,
	.uninit=nowindow_display_uninit,
	.pollevent=nowindow_display_pollevent
};
...
#elif defined(NOWINDOW_DISPLAY)
static MSDisplayDesc *default_display_desc=&ms_nowindow_display_desc;
#elif defined(WIN32)
...
static void video_out_uninit(MSFilter *f){
	...
	if (obj->sws1!=NULL){
		ms_video_scalercontext_free(obj->sws1);
		obj->sws1=NULL;
	}
	if (obj->sws2!=NULL){
		ms_video_scalercontext_free(obj->sws2);
		obj->sws2=NULL;
	}
...
static void video_out_process(MSFilter *f){
	...
	if (f->inputs[1]!=NULL && (inm=ms_queue_peek_last(f->inputs[1]))!=0) {
		if (obj->corner==-1){
			...
			if (yuv_buf_init_from_mblk(&src,inm)==0){
				...
				if (obj->sws2==NULL){
					obj->sws2=ms_video_scalercontext_init(src.w,src.h,MS_YUV420P,
											 obj->fbuf_selfview.w,obj->fbuf_selfview.h,MS_YUV420P,
											 MS_YUVFAST, NULL, NULL, NULL);
				}
				ms_display_lock(obj->display);
				if (ms_video_scalercontext_convert(obj->sws2,src.planes,src.strides, 0,
	...
	if (f->inputs[0]!=NULL && (inm=ms_queue_peek_last(f->inputs[0]))!=0) {
		...
		if (yuv_buf_init_from_mblk(&src,inm)==0){
            ...
            /* live change of incoming stream size */
            if (obj->sws1!=NULL && (obj->prevsize.width!=src.w || obj->prevsize.height!=src.h))
            {
                video_out_prepare(f);
            }
            obj->prevsize.width=src.w;
            obj->prevsize.height=src.h;
            /* end of live change of incoming stream size */
            ...
            ratioh=src.h;
            reduce(&ratiow, &ratioh);
            wtmp = obj->fbuf.w/ratiow*ratiow;
            htmp = obj->fbuf.h/ratioh*ratioh;
            ...
            x=((int)(((float)obj->fbuf.w*0.5)-((float)w/2))) & ~0x1;
            y=((int)(((float)obj->fbuf.h*0.5)-((float)h/2))) & ~0x1;
            
            dest.w=w;
            dest.h=h;
            dest.planes[0]=obj->fbuf.planes[0]+(y*obj->fbuf.strides[0])+x;
            dest.planes[1]=obj->fbuf.planes[1]+((y/2)*obj->fbuf.strides[1])+(x/2);
            dest.planes[2]=obj->fbuf.planes[2]+((y/2)*obj->fbuf.strides[2])+(x/2);
            dest.planes[3]=NULL;
            dest.strides[0]=w;
            dest.strides[1]=w/2;
            dest.strides[2]=w/2;
            dest.strides[3]=0;
 			...
 			if (obj->sws1==NULL){
				yuv_buf_background(&obj->fbuf, obj->background_color[0], obj->background_color[1], obj->background_color[2]);
				obj->sws1=ms_video_scalercontext_init(src.w,src.h,MS_YUV420P,
                                            dest.w,dest.h,MS_YUV420P,
                                            MS_YUVFAST, NULL, NULL, NULL);
			}
            
			ms_display_lock(obj->display);
			if (ms_video_scalercontext_convert(obj->sws1,src.planes,src.strides, 0,
            			src.h, dest.planes, obj->fbuf.strides)<0){
				ms_error("Error in ms_video_scalercontext_convert().");
			}
...
MSFilterDesc ms_video_out_desc={
	...
	NULL,
	2,
#if defined(NOWINDOW_DISPLAY_WITH_OUTPUT)
	1,
#else
	0,
#endif
