...
static int _vfw_engine_connect(VfwEngine *obj){
	...
	HWND hwnd=capCreateCaptureWindow( L"Capture Window",WS_CHILD /* WS_OVERLAPPED */
...
static VfwEngine * vfw_engine_new(int i){
	...
	if (capGetDriverDescription(i, (LPWSTR)dev, sizeof (dev),
		(LPWSTR)ver, sizeof (ver))){
		obj->devidx=i;
		if (_vfw_engine_connect(obj)==-1){
			ms_free(obj);
			return NULL;
		}
		WideCharToMultiByte(CP_UTF8,0,(LPWSTR)dev,-1,obj->dev,sizeof(obj->dev),0,0);
		engines[i]=obj;
		return obj;
	}
	return NULL;
