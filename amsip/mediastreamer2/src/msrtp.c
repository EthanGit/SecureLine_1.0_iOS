...
static uint32_t get_cur_timestamp(MSFilter * f, uint32_t packet_ts)
{
	...
	else if (diff < -(delta * 5)) {
		d->tsoff = curts - packet_ts;
		netts = packet_ts + d->tsoff;
		ms_message("ticker back in time diff=%i", diff);
...
static int send_dtmf(MSFilter * f, uint32_t timestamp_start, uint32_t current_timestamp)
{
	...
	if (RTP_TIMESTAMP_IS_NEWER_THAN(current_timestamp, d->skip_until)) {
		...
		tmp=copymsg(m1);
		rtp_session_sendm_with_ts(d->session,tmp,timestamp_start+160);
		d->session->rtp.snd_seq--;
		tmp=copymsg(m1);
		rtp_session_sendm_with_ts(d->session,tmp,timestamp_start+160);
		d->session->rtp.snd_seq--;
		rtp_session_sendm_with_ts(d->session,m1,timestamp_start+160);
	}
	else {
...
static void sender_process(MSFilter * f)
{
	...
	if (f->inputs[0]->q.q_mcount>3)
	{
		ms_warning("RTP burst to be sent: %i", f->inputs[0]->q.q_mcount);
	}
...
static int sender_get_statistics(MSFilter *f, void *arg){
	int i;
...
static MSFilterMethod sender_methods[] = {
	...
	{MS_FILTER_GET_STATISTICS, sender_get_statistics },
	{0, NULL}
};
...
static void receiver_uninit(MSFilter * f){
	ReceiverData *d = (ReceiverData *) f->data;
	ms_warning("incoming RTP burst detected during conversation: %i", d->rtp_burst);
	ms_free(d);
}
...
static int receiver_set_session(MSFilter * f, void *arg)
{
	ReceiverData *d = (ReceiverData *) f->data;
	RtpSession *s = (RtpSession *) arg;
	PayloadType *pt = rtp_profile_get_payload(rtp_session_get_recv_profile(s),
...
static int receiver_get_sr(MSFilter *f, void *arg){
	...
	pt=rtp_profile_get_payload(rtp_session_get_recv_profile(d->session),
...
static void receiver_process(MSFilter * f)
{
	...
	while ((m = rtp_session_recvm_with_ts(d->session, timestamp)) != NULL) {
		...
		if (d->max_inbuffer<d->session->rtp.rq.q_mcount)
			d->max_inbuffer = d->session->rtp.rq.q_mcount;
		if (rtp_session_get_last_recv_error_code(d->session)>0)
		{
			rtp_session_clear_recv_error_code(d->session);
			d->errs++;
		}
	}
	
	if (f->outputs[0]->q.q_mcount>5)
	{
		//ms_warning("RTP burst detected: %i", f->outputs[0]->q.q_mcount);
		d->rtp_burst++;
	}
}


static int receiver_get_statistics(MSFilter *f, void *arg){
	ReceiverData *d = (ReceiverData *) f->data;
	ms_warning("filter: %s[%i->%i]", f->desc->name,
		f->desc->ninputs, f->desc->noutputs);
	if (d->session!=NULL)
	{
		ms_warning("filter: %s RTP=%i packets -> out[0]=%i", f->desc->name, 
			d->session->rtp.rq.q_mcount,
			(f->outputs[0]!=NULL)?f->outputs[0]->q.q_mcount:-1);
	}
	ms_warning("filter: %s max_inbuffer=%i packets // sendto errs=%i", f->desc->name, 
		d->max_inbuffer,
		d->errs);
...
static MSFilterMethod receiver_methods[] = {
	...
	{	MS_FILTER_GET_STATISTICS, receiver_get_statistics },
	{	0, NULL}
};
