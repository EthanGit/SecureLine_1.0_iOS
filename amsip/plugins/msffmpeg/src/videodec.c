...
static void dec_init(MSFilter *f, enum CodecID cid){
	FFMpegDecState *s=(FFMpegDecState *)ms_new0(FFMpegDecState,1);
...
static void dec_uninit(MSFilter *f){
	FFMpegDecState *s=(FFMpegDecState*)f->data;
...
static mblk_t * skip_rfc2190_header(FFMpegDecState *s,mblk_t *inm){
	if (msgdsize(inm) >= 4) {
		uint8_t *ph = inm->b_rptr;
		uint8_t sbit = (ph[0] & 0x56) >> 3;
		uint8_t ebit = (ph[0] & 0x07);
		int F = (ph[0]>>7) & 0x1;
		int P = (ph[0]>>6) & 0x1;
		...
		if (F == 0) {
			if ((ph[1] & 0x10)==0)
				ms_message("receiving H263 I-frame");
		} else {
			if ((ph[4] & 0x80)==0)
				ms_message("receiving H263 I-frame");
		}
		if (sbit!=0 && s->input) { //mix previous final bit with current one
			uint8_t m = (0xFF >> sbit);
			mblk_t *mp = s->input;
			while ( mp->b_cont != NULL ) mp = mp->b_cont;
			mp->b_wptr[-1] |= (inm->b_rptr[0] & m);
			inm->b_rptr++; //skip byte that have been copied
		}
		if (ebit!=0) {
			inm->b_wptr[-1] = (inm->b_wptr[-1] >> ebit) << ebit;
		}
	} else {
		freemsg(inm);
		inm=NULL;
...
static mblk_t * parse_snow_header(FFMpegDecState *s,mblk_t *inm){
	if (msgdsize(inm) >= 4){
...
static mblk_t *
read_rfc2435_header(FFMpegDecState *s,mblk_t *inm)
{
...
static mblk_t *get_as_yuvmsg(MSFilter *f, FFMpegDecState *s, AVFrame *orig){
...
static void dec_process_frame(MSFilter *f, mblk_t *inm){
	...
	if (strcmp(f->desc->enc_fmt, "H263-1998")==0) inm=skip_rfc2429_header(inm);
	else if (strcmp(f->desc->enc_fmt, "H263")==0) inm=skip_rfc2190_header(s, inm);
	else if (s->codec==CODEC_ID_SNOW && s->input==NULL) inm=parse_snow_header(s,inm);
	else if (s->codec==CODEC_ID_MJPEG && strcmp(f->desc->enc_fmt, "JPEG")==0) inm=read_rfc2435_header(s,inm);
	if (inm){
