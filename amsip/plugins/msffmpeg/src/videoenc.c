...
#include <mediastreamer2/rfc2429.h>
...
#define ENABLE_LOG_FFMPEG
#ifdef ENABLE_LOG_FFMPEG

static void ms_ffmpeg_log_callback(void* ptr, int level, const char* fmt, va_list vl)
{
...
static int enc_set_fps(MSFilter *f, void *arg){
	FFMpegEncState *s=(FFMpegEncState*)f->data;
	s->fps=*(float*)arg;
	return 0;
}
...
static bool_t parse_video_fmtp(const char *fmtp, float *fps, MSVideoSize *vsize){
	...
	if ((equal=strchr(tmp,'='))!=NULL){
		...
		*equal='\0';
		...
		if (divider!=0){
			float newfps=(float)(29.97/divider);
...
{
	FFMpegEncState *s=(FFMpegEncState *)ms_new(FFMpegEncState,1);
	f->data=s;
...
static void enc_h263_old_init(MSFilter *f){
	enc_init(f,CODEC_ID_H263);
}
...
static void prepare(FFMpegEncState *s){
	...
	c->bit_rate=(int)(((float)s->maxbr)*0.9);
	...
	if (s->codec!=CODEC_ID_SNOW && s->maxbr<256000){
		...
		c->rc_max_rate=(int)((float)s->maxbr*0.95);
		c->rc_min_rate=0;
...
static void prepare_h263(FFMpegEncState *s){
	AVCodecContext *c=&s->av_context;
...
#if LIBAVUTIL_VERSION_INT >= AV_VERSION_INT(51,13,0)
		av_opt_set(c, "umv", "1", 0);
		av_opt_set(c, "structured_slices", "1", 0); /* should this be "slice" ? */
#else
	...
	s->av_context.thread_count = 1;
}

static void prepare_mpeg4(FFMpegEncState *s){
	AVCodecContext *c=&s->av_context;
	...
	//av_opt_set(c, "gmc", "1", 0);
	c->flags|=CODEC_FLAG_4MV;

	//av_opt_set(c, "gmc", "1", 0);
	c->flags|=CODEC_FLAG_GMC; /* unsused? todo: remove? */
	//av_opt_set(c, "loop", "1", 0);
...
#if 0
static void enc_set_rc(FFMpegEncState *s, AVCodecContext *c){
...
static void enc_preprocess(MSFilter *f){
	...
	if (error!=0) {
        //H263 is limited -> Valid sizes are 128x96, 176x144, 352x288, 704x576, and 1408x1152. Try H.263+.
...
static void rfc2190_generate_packets(MSFilter *f, FFMpegEncState *s, mblk_t *frame, uint32_t timestamp){
	mblk_t *packet=NULL;
...
static void mpeg4_fragment_and_send(MSFilter *f,FFMpegEncState *s,mblk_t *frame, uint32_t timestamp){
	uint8_t *rptr;
	mblk_t *packet=NULL;
	int len;
...
static void rfc4629_generate_follow_on_packets(MSFilter *f, FFMpegEncState *s, mblk_t *frame, uint32_t timestamp, uint8_t *psc, uint8_t *end, bool_t last_packet){
	mblk_t *packet;
	int len=(int)(end-psc);
...
static void mjpeg_fragment_and_send(MSFilter *f,FFMpegEncState *s,mblk_t *frame, uint32_t timestamp,
...
static void split_and_send(MSFilter *f, FFMpegEncState *s, mblk_t *frame){
	uint8_t *lastpsc;
	uint8_t *psc;
	...
	if (strcmp(f->desc->enc_fmt, "H263-1998")==0){
	...
	}else if (strcmp(f->desc->enc_fmt, "H263")==0){
...
static void process_frame(MSFilter *f, mblk_t *inm){
	...
	int comp_buf_sz=(int)(comp_buf->b_datap->db_lim-comp_buf->b_datap->db_base);
	...
	pict.pts=INT64_C(0x8000000000000000);
...
static void enc_process(MSFilter *f){
	...
	while((inm=ms_queue_get(f->inputs[0]))!=0){
        ...
        int w = mblk_get_video_width(inm);
        int h = mblk_get_video_height(inm);
        if (s->vsize.width!=w || s->vsize.height!=h)
        {
            s->vsize.width=w;
            s->vsize.height=h;
            enc_postprocess(f);
            enc_preprocess(f);
            ms_message("new size for encoder images applied (%ix%i)", w, h);
            if (s->av_context.codec==NULL) {
                freemsg(inm);
                ms_queue_flush(f->inputs[0]);
                return;
...
GLOBAL_LINKAGE void libmsffmpeg_init(void);

GLOBAL_LINKAGE void libmsffmpeg_init(void){
	ms_message("msffmpeg: ffmpeg H263-1998/H263/MP4V-ES/MJPEG plugin");
	ms_message("  version: [%d.%d.%d] ",
		LIBAVCODEC_VERSION_MAJOR, LIBAVCODEC_VERSION_MINOR, LIBAVCODEC_VERSION_MICRO );
	ms_message("  license: %s ",
		avcodec_license() );

	ms_filter_register(&ms_mpeg4_dec_desc);
	ms_filter_register(&ms_h263_dec_desc);
	ms_filter_register(&ms_h263_old_dec_desc);
	ms_filter_register(&ms_snow_dec_desc);
	ms_filter_register(&ms_jpeg_dec_desc);
	ms_filter_register(&ms_mjpeg_dec_desc);
