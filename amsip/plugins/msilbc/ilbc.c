...
static void enc_init(MSFilter *f){
	IlbcEncState *s=ms_new(IlbcEncState,1);
...
static void enc_uninit(MSFilter *f){
	IlbcEncState *s=(IlbcEncState*)f->data;
...
static void dec_init(MSFilter *f){
	IlbcDecState *s=ms_new(IlbcDecState,1);
	s->nsamples=0;
	s->nbytes=0;
	s->ms_per_frame=0;
	f->data=s;
...
static void dec_process(MSFilter *f){
	IlbcDecState *s=(IlbcDecState*)f->data;
	mblk_t *im,*om;
	int nbytes;
