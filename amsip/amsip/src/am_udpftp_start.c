...
int
_calls_start_udpftp_with_id(int tid_for_offer, int did,
							osip_message_t * answer)
{
	...
	OSIP_TRACE(osip_trace
			   (__FILE__, __LINE__, OSIP_WARNING, NULL,
				"Start udpftp ressource cid=%i did=%i\n", ca->cid,
				ca->did));

	return _calls_start_udpftp_with_sipanswer(ca, tid_for_offer, answer,
											  1);
}

int
_calls_start_udpftp_with_sipanswer(am_call_t * ca, int tid_for_offer,
								   osip_message_t * answer,
								   int local_is_offer)
{
	int i;
	...
	i = _calls_start_udpftp_from_sdpmessage(ca, offer_sdp, answer_sdp,
											local_is_offer);
	sdp_message_free(offer_sdp);
...
int
_calls_start_udpftp_from_sipmessage(am_call_t * ca, osip_message_t * offer,
									osip_message_t * answer,
									int local_is_offer)
{
	int i;
...
int
_calls_start_udpftp_from_sdpmessage(am_call_t * ca,
									...
									int local_is_offer)
{
	...
	answer_port = -1;
	offer_port = -1;
	_calls_get_ip_port(answer_sdp, "x-udpftp", answer_ip, &answer_port);
	_calls_get_ip_port(offer_sdp, "x-udpftp", offer_ip, &offer_port);
	...
	offer_med = eXosip_get_media(offer_sdp, "x-udpftp");
	answer_med = eXosip_get_media(answer_sdp, "x-udpftp");
	...
	if (udpftp_local_sendrecv == _INACTIVE)
		remote_sendrecv = _INACTIVE;
	if (remote_sendrecv == _INACTIVE)
		udpftp_local_sendrecv = _INACTIVE;

	if (udpftp_local_sendrecv == _SENDRECV) {
		if (remote_sendrecv == _SENDONLY)
			udpftp_local_sendrecv = _RECVONLY;
		else if (remote_sendrecv == _RECVONLY)
			udpftp_local_sendrecv = _SENDONLY;
	}

	if (udpftp_local_sendrecv == _SENDONLY
		|| udpftp_local_sendrecv == _SENDRECV) {
		if (local_is_offer == 0) {
	...
	ca->udpftp_local_sendrecv = udpftp_local_sendrecv;
	if (local_is_offer == 0) {
		ca->udpftp_rtp_in_direct_mode = 0;
		snprintf(ca->udpftp_rtp_remote_addr, 256, "%s:%i", answer_ip,
				 answer_port);
		if (0 ==
			os_udpftp_start(ca, answer_sdp, offer_sdp, /* local */
							offer_sdp,
							/* remote */ answer_sdp, offer_port,
							answer_ip, answer_port, setup_passive)) {
			ca->enable_udpftp = 1;	/* udpftp is started */
		}
	} else {
		ca->udpftp_rtp_in_direct_mode = 0;
		snprintf(ca->udpftp_rtp_remote_addr, 256, "%s:%i", offer_ip,
				 offer_port);
		if (0 ==
			os_udpftp_start(ca, answer_sdp, offer_sdp, /* local */
							answer_sdp,
