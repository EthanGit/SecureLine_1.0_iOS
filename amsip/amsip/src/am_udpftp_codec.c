...
PPL_DECLARE (int)
am_udpftp_codec_info_modify(am_udpftp_codec_info_t * codec, int i)
{
	if (codec == NULL)
	...
	if (codec->enable > 0) {
		...
		if (!ms_filter_codec_supported(codec->name)) {
			...
			OSIP_TRACE(osip_trace(__FILE__, __LINE__, OSIP_WARNING, NULL,
								  ...
								  codec->name));
			memcpy(&_antisipc.udpftp_codecs[i], codec,
				   sizeof(am_udpftp_codec_info_t));
...
PPL_DECLARE (int)
am_udpftp_codec_attr_modify(am_udpftp_codec_attr_t * codec_attr)
{
	if (codec_attr == NULL)
		return AMSIP_BADPARAMETER;
	memcpy(&_antisipc.udpftp_codec_attr, codec_attr,
		   sizeof(am_udpftp_codec_attr_t));
	if (_antisipc.udpftp_codec_attr.download_bandwidth <= 0)
		_antisipc.udpftp_codec_attr.download_bandwidth = 256;
	return AMSIP_SUCCESS;
}
...
int _am_udpftp_codec_get_definition(char *name)
{
	...
	for (i = 0; i < 5; i++) {
		if (osip_strcasecmp(_antisipc.udpftp_codecs[i].name, name) == 0)
			return i;
	}
	return AMSIP_NOTFOUND;
}
