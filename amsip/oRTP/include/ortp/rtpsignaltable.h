...
struct _RtpSignalTable
{
	...
	uintptr_t user_data[RTP_CALLBACK_TABLE_MAX_ENTRIES];
...
int rtp_signal_table_add(RtpSignalTable *table,RtpCallback cb, uintptr_t user_data);

void rtp_signal_table_emit(RtpSignalTable *table);

/* emit but with a second arg */
void rtp_signal_table_emit2(RtpSignalTable *table, uintptr_t arg);

/* emit but with a third arg */
void rtp_signal_table_emit3(RtpSignalTable *table, uintptr_t arg1, uintptr_t arg2);
