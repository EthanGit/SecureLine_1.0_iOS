...
static void report_block_parse(RtpSession *session, report_block_t *rb, struct timeval rcv_time_tv)
{   		
  ...
  if ( rb->ssrc != session->snd.ssrc )
...

    {
      ...
      rcv_time_msw = (uint32_t)rcv_time_tv.tv_sec;
...
void rtp_session_rtcp_parse(RtpSession *session, mblk_t *mp)
{
  ...
  if (msgsize < RTCP_COMMON_HEADER_SIZE)
    {
      ortp_warning("Receiving too short RTCP packet... discarded");
      return;
    }
  ...
  while (msgsize >= RTCP_COMMON_HEADER_SIZE)
    {
      ...
      if (rtcp->version!=2)
        {
          ortp_warning("Receiving RTCP packet with version number !=2...discarded");
          return;
        }
      ...
      if (rtcp_pk_size > msgsize)
        {
          ortp_warning("Receiving RTCP packet shorter than the specified length.. discared");
          return;
        }
	...
	case RTCP_SR:

	  {
	    ...
	    ortp_message("Receiving RTCP SR");
	    if ( ntohl(sr->ssrc) != session->rcv.ssrc )
	      {
		ortp_warning("Receiving RTCP sr packet from unknown ssrc.. discarded");		
		return;
	      }	    

	    if (msgsize < RTCP_COMMON_HEADER_SIZE + RTCP_SSRC_FIELD_SIZE + RTCP_SENDER_INFO_SIZE + (RTCP_REPORT_BLOCK_SIZE*sr->ch.rc))
	      {
		ortp_warning("Receiving too short RTCP sr packet... discarded");
		return;
	      }	    
	...
	case RTCP_RR:

	  {
	    ...
	    report_block_t *rb;
	    int i;
	    ortp_message("Receiving RTCP RR");

        if (session->rcv.ssrc == 0)
          {
            /* rcv.ssrc is not set, so we adopt the incoming one */
            session->rcv.ssrc = ntohl(rr->ssrc);
            ortp_warning("Receiving RTCP rr packet from unknown ssrc.. adopting %X", session->rcv.ssrc);
          }
	    else if ( ntohl(rr->ssrc) != session->rcv.ssrc )
          {
	    if (session->mode==RTP_SESSION_SENDONLY) /* session->snd.ssrc==0xAC288726) */
	      {
		session->rcv.ssrc = ntohl(rr->ssrc);
		ortp_warning("Receiving RTCP rr packet from unknown ssrc.. fix/adopting %X", session->rcv.ssrc);
	      }
            else
	      {
		ortp_warning("Receiving RTCP rr packet from unknown ssrc.. discarded %X %X", ntohl(rr->ssrc), session->rcv.ssrc);
		return;
	      }
          }
	...
	case RTCP_SDES: 
	  /* to be implemented */
	  ortp_message("Receiving RTCP SDES");
	break;
	...
	ortp_message("Receiving RTCP BYE");
        if (reason_space_len < 0) {
            ortp_warning("Receiving too short RTCP BYE packet... discarded");
            return;
        }
            ...
            if (session->on_rtcp_bye.count > 0) {
                ...
                if (reason_space_len > 1) {
                    ...
                    if (reasonbuf[0] <= reason_space_len-1)
                        ...
                        ortp_warning("Incorrect RTCP BYE reason length");
                }
                ...
                ortp_warning("Got RTCP BYE without RTCP BYE handler");
            }
        } else {
            ortp_warning("No SSRC in the RTCP BYE packet matched our rcv.ssrc.");
        }
	    break;
      }
	...
	case RTCP_APP:
	  /* to be implemented */
	  ortp_message("Receiving RTCP app");
	  break;

	  
	default:

	  ortp_warning("Receiving unknown RTCP packet type... discarded");
	  return;
