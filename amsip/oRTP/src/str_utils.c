...
mblk_t *dupb(mblk_t *mp)
{
	...
	newm->reserved3=mp->reserved3;
	newm->reserved4=mp->reserved4;
	newm->reserved5=mp->reserved5;
...
void msgpullup(mblk_t *mp,int len)
{
	...
	if (mp==NULL) ortp_fatal("mp must not be NULL here");
	...
	while(wlen<len && mp!=NULL){
		...
		//ortp_message("remaining %i-%i=%i", len, wlen, remain);
		//ortp_message("total left %i", mlen);
		if (mlen<=remain){
