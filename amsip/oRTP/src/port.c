...
int __ortp_thread_join(ortp_thread_t thread, void **ptr){
	...
	if (err!=0) {
		ortp_error("pthread_join error: %s",ortp_strerror(err));
	}
	return err;
}
...
ortp_socket_t ortp_server_pipe_create(const char *name){
	...
	if (bind(sock,(struct sockaddr*)&sa,sizeof(sa))!=0){
		ortp_error("Failed to bind command unix socket: %s",ortp_strerror(errno));
		return -1;
	}
	listen(sock,1);
	return sock;
...
int ortp_pipe_read(ortp_pipe_t p, uint8_t *buf, int len){
	...
	if (ReadFile(p,buf,len,&ret,NULL))
		return ret;
	/*ortp_error("Could not read from pipe: %s",ortp_strerror(GetLastError()));*/
	return -1;
}
...
int ortp_pipe_write(ortp_pipe_t p, const uint8_t *buf, int len){
	...
	if (WriteFile(p,buf,len,&ret,NULL))
		return ret;
	/*ortp_error("Could not write to pipe: %s",ortp_strerror(GetLastError()));*/
	return -1;
}
...
char * ortp_strerror(DWORD value) {
