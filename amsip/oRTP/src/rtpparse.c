...
static void queue_packet(queue_t *q, int maxrqsz, mblk_t *mp, rtp_header_t *rtp, int *discarded)
{
	...
	int header_size;
	*discarded=0;
	header_size=RTP_FIXED_HEADER_SIZE+ (4*rtp->cc)+(4*rtp->extbit);
	
	if (rtp->extbit)
	{
		uint16_t *ext_buf = (uint16_t *)(mp->b_rptr+header_size-2);
		header_size += (ntohs(ext_buf[0])*4);
	}
...
void rtp_session_rtp_parse(RtpSession *session, mblk_t *mp, uint32_t local_str_ts, struct sockaddr *addr, socklen_t addrlen)
{
	...
	if (!session->use_connect){
		if (session->rtp.socket>0 && session->symmetric_rtp && stats->packet_recv==0){
			/* store the sender rtp address to do symmetric RTP */
			if (addrlen!=session->rtp.rem_addrlen
				|| memcmp(&session->rtp.rem_addr,addr, addrlen)!=0)
			{
				ortp_message("Symmetric RTP with initial RTP/data packet! %i", session->rtp.loc_port);
			...
			if (session->rtp.socket>0 && session->symmetric_rtp){
					...
					|| memcmp(&session->rtp.rem_addr,addr, addrlen)!=0)
				{
					if (!(session->flags & RTP_SESSION_FIRST_PACKET_DELIVERED)){
						ortp_message("Symmetric RTP with initial RTP/delivered packet! %i", session->rtp.loc_port);
						memcpy(&session->rtp.rem_addr,addr,addrlen);
						session->rtp.rem_addrlen=addrlen;
						rtp_session_set_dscp(session, -1);
					}
	...
	if (session->hw_recv_pt!=rtp->paytype){
		...
		if (session->flags & RTP_SESSION_FIRST_PACKET_DELIVERED) {
			if (rtp->timestamp==0 && rtp->seq_number==0)
			{
				/* interop issue with SPIRIT sending ARS packets */
				/* drop */
				ortp_warning("rtp_parse: drop RTP packet (timestamp=0 seqnumber=0)");
				freemsg(mp);
				return;
			}
		}
	...
	if (session->flags & RTP_SESSION_FIRST_PACKET_DELIVERED) {
		...
		if (RTP_TIMESTAMP_IS_NEWER_THAN(rtp->timestamp,session->rtp.rcv_last_ts+session->rtp.ts_jump)){
			...
			*/
			...
			ortp_warning("rtp_parse: discarding too old packet (ts=%i)",rtp->timestamp);
			freemsg(mp);
			stats->outoftime++;
