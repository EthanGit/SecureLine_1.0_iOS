...
extern void tc_qos_delete_flow(RtpSession *session);
...
static void wait_point_init(WaitPoint *wp){
	ortp_mutex_init(&wp->lock,NULL);
	ortp_cond_init(&wp->cond,NULL);
	wp->time=0;
	wp->wakeup=FALSE;
}
static void wait_point_uninit(WaitPoint *wp){
	...
	*rejected=0;
	...
	while ((tmp=qfirst(q))!=NULL)
	{
		...
		if ( RTP_TIMESTAMP_IS_NEWER_THAN(timestamp,tmprtp->timestamp) )
		{
			...
			if (ret!=NULL && ((tmprtp->timestamp-ts_found)<=80)) { /* hack for ptime lower than interval */
				/* we've found two packets with same timestamp. return the first one */
				break;
			}
			if (old!=NULL) {
				ortp_warning("rtp_getq: discarding too old packet with ts=%i",ts_found);
				(*rejected)++;
				freemsg(old);
			}
...
static mblk_t *rtp_getq_permissive(queue_t *q,uint32_t timestamp, int *rejected)
{
	mblk_t *tmp,*ret=NULL;
	rtp_header_t *tmprtp;
...
rtp_session_init (RtpSession * session, int mode)
{
	...
	session->rtp.rtcp_report_snt_interval = RTCP_DEFAULT_REPORT_INTERVAL*90000; //needs to be initialized - if we don't receive anyting.
...
rtp_session_signal_connect (RtpSession * session, const char *signal_name,
			    RtpCallback cb, uintptr_t user_data)
{
	OList *elem;
...
void rtp_session_release_sockets(RtpSession *session){
	if (sockisvalid(session->rtp.socket)) close_socket (session->rtp.socket);
	if (sockisvalid(session->rtcp.socket)) close_socket (session->rtcp.socket);
	session->rtp.socket=-1;
	session->rtcp.socket=-1;
	
#if 0
	/* we may want only to reset the stream and keep the transport */
...
ortp_socket_t rtp_session_get_rtp_socket(const RtpSession *session){
	return rtp_session_using_transport(session, rtp) ? (session->rtp.tr->t_rtp_getsocket)(session) : session->rtp.socket;
}

ortp_socket_t rtp_session_get_rtcp_socket(const RtpSession *session){
	return rtp_session_using_transport(session, rtcp) ? (session->rtcp.tr->t_rtcp_getsocket)(session) : session->rtcp.socket;
}
...
int rtp_get_payload(mblk_t *packet, unsigned char **start){
	unsigned char *tmp;
	int header_len=RTP_FIXED_HEADER_SIZE+(rtp_get_cc(packet)*4)+(rtp_get_extbit(packet)*4);
	if (rtp_get_extbit(packet))
	{
		uint16_t *ext_buf = (uint16_t *)(packet->b_rptr+header_len-2);
		header_len += (ntohs(ext_buf[0])*4);
	}
