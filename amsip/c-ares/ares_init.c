...
DhcpNameServer
  ...
  if (_res.nscount == 0)
    return ARES_SUCCESS; /* use localhost DNS server */
  nservers = _res.nscount;
  servers = calloc(_res.nscount, sizeof(struct server_state));
  if (!servers)
    return ARES_ENOMEM;
  ...
  for (i = 0; i < nservers; i++)
  {
	  ...
	  {
		  servers[i].addr.family = _res.nsaddr_list[i].sin_family;
		  memcpy(&servers[i].addr.addrV4, &_res.nsaddr_list[i].sin_addr, sizeof(struct in_addr));		  
	  }
  }
