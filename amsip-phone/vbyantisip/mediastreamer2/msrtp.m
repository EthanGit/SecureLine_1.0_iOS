...
static void sender_process(MSFilter * f)
{
	...
	if (f->inputs[0]->q.q_mcount>2)
	{
		NSLog(@"RTP burst to be sent: %i", f->inputs[0]->q.q_mcount);
	}
...
static void receiver_uninit(MSFilter * f){
	ReceiverData *d = (ReceiverData *) f->data;
	NSLog(@"incoming RTP burst detected during conversation: %i", d->rtp_burst);
	ms_free(d);
}
...
static void receiver_process(MSFilter * f)
{
	...
	{
		NSLog(@"RTP burst detected: %i", f->outputs[0]->q.q_mcount);
		d->rtp_burst++;
	}
}
